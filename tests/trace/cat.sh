#!/bin/sh
# Syscall-trace test for uolt-cat: it may only open/read/close files and write
...
if [ "$os" = "Linux" ] && command -v strace >/dev/null 2>&1; then
    ...
    echo "PASS trace/cat (strace)"; exit 0
    ...
    if dtruss "$BIN" "$tmp/in" >/dev/null 2>"$tmp/trace"; then
        ...
        echo "PASS trace/cat (dtruss)"; exit 0
    fi
    echo "SKIP trace/cat: dtruss unavailable (needs sudo / SIP)"; exit 0
fi

echo "SKIP trace/cat: no tracer on this platform"; exit 0
